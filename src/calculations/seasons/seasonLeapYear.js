...
export default function seasonLeapYear() {
  const dayOfYear = calcDayOfYear();

  let season;
  if (dayOfYear > December.normal || dayOfYear <= March.leapYear) {
    season = "VERANO";
  }
  if (dayOfYear > March.leapYear && dayOfYear <= June.leapYear) {
    season = "OTOÑO";
  }
  if (dayOfYear > June.leapYear && dayOfYear <= September.leapYear) {
    season = "INVIERNO";
  }
  if (dayOfYear > September.leapYear && dayOfYear <= December.leapYear) {
    season = "PRIMAVERA";
  }
  return season;
}
